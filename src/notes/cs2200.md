...
We define $\equiv_M$ to be the equivalence relation gotten when any two strings end up in the same equivalence class in the language accepted by $M$.

We define $\equiv_L$ to be the equivalence relation gotten when any two strings $x$ and $y$ s.t. $\hat{\delta}\left(q_0, x\right) = \hat{\delta}\left(q_0, y\right)$
...
$$\begin{align}
&\forall x,y, x\equiv y \rightarrow x\equiv_L y \left(\forall z \in \Sigma^* , xz\in L \iff yz\in L\right)
\\
